...
static opt::OptionsDescription DescForPsEstablish(const std::string &subCommand, const CmdEntry &entry)
{
    ...
    std::string example2 = "IPv4v6 --emergency";
                                                    ...
                                                    std::string &error)
{
    ...
    else if (subCmd == "ps-establish")
    {
        ...
        if (type == "IPv4" || type == "ipv4" || type == "IPV4" || type == "Ipv4" || type == "IpV4")
            cmd->sessionType = nas::EPduSessionType::IPV4;
        else if (type == "IPv6" || type == "ipv6" || type == "IPV6" || type == "Ipv6" || type == "IpV6")
            cmd->sessionType = nas::EPduSessionType::IPV6;
        else if (type == "IPv4v6" || type == "ipv4v6" || type == "IPV4V6" || type == "Ipv4v6")
            cmd->sessionType = nas::EPduSessionType::IPV4V6;
        else
